...
public class BoggleGame {
    ...
    private String randomizeLetters(int size){
        ...
        if(size == 4){
           dice  = new ArrayList<String>(List.of(dice_small_grid));
        }
        else{
            dice = new ArrayList<String>(List.of(dice_big_grid));
        }
        String s = "";
        Collections.shuffle(dice, random);
        for(String d: dice)
        {
            s += d.charAt(random.nextInt(d.length()));
        }
        return s;
    }
     ...
     * -- At every Position on the grid: # loop through the 2d array
    ...
    private void findAllWords(Map<String,ArrayList<Position>> allWords, Dictionary boggleDict, BoggleGrid boggleGrid) {
        // Perform a word search on every cell
        for(int i = 0; i < boggleGrid.numRows(); i++){
            for(int j = 0; j < boggleGrid.numCols(); j++){
                search(allWords, boggleDict, boggleGrid, new ArrayList<Position>(List.of(new Position(i, j))), "");
            }
        }
    }

    public boolean outsideGrid(BoggleGrid boggleGrid, int i, int j)
    {
        return i<0 || j<0 || i >= boggleGrid.numRows() || j >= boggleGrid.numCols();
    }

    public void search(Map<String,ArrayList<Position>> allWords, Dictionary boggleDict, BoggleGrid boggleGrid,ArrayList<Position> wordPos, String word){
        Position currPos = wordPos.get(wordPos.size()-1);
        int i = currPos.getRow();
        int j = currPos.getCol();
        word += boggleGrid.getCharAt(i,j);

        // Check if the dictionary contains the word
        if(word.length() >= 4 && boggleDict.containsWord(word)) allWords.put(word, wordPos);

        // If the word is a valid prefix find an adjacent letter to make a new word
        if(!boggleDict.isPrefix(word)) return;

        boolean [][] visited = new boolean[boggleGrid.numRows()][boggleGrid.numCols()];
        for(Position pos: wordPos)
        {
            visited[pos.getRow()][pos.getCol()] = true;
        }

        // Each of the possible directions
        Position [] directions = {new Position(i,j+1), new Position(i+1,j+1)
                , new Position(i+1,j), new Position(i+1,j-1), new Position(i,j-1)
                , new Position(i-1,j-1), new Position(i-1,j), new Position(i-1,j+1)};

        ArrayList<Position> wordPosUpdated;
        for(Position direction: directions) {
            // Only go in directions within the grid
            if(outsideGrid(boggleGrid, direction.getRow(), direction.getCol())) continue;
            if(visited[direction.getRow()][direction.getCol()]) continue;
            wordPosUpdated = new ArrayList<Position>(wordPos);
            wordPosUpdated.add(direction);
            search(allWords, boggleDict, boggleGrid, wordPosUpdated, word);
        }
    }
    ...
    private void humanMove(BoggleGrid board, Map<String,ArrayList<Position>> allWords){
        ...
        while(true) {
            ...
            if(allWords.containsKey(input) && !gameStats.getPlayerWords().contains(input)){
                gameStats.addWord(input, BoggleStats.Player.Human);
            }
    ...
    private void computerMove(Map<String,ArrayList<Position>> all_words){
        Set<String> set =  all_words.keySet();
        set.removeAll(gameStats.getPlayerWords());
        for(String s: set){
